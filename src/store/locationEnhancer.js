...
const clusterLocationPoints = (locationHistory) => {
  ...
  Object.keys(locationHistory).forEach((user) => {
    ...
    Object.keys(locationHistory[user]).forEach((device) => {
      ...
      const processedLocations = processClusterToLocations(mergedClusters, locations);
      
      console.log(`[LocationEnhancer] ${user}/${device}: Found ${clusters.length} clusters, merged to ${mergedClusters.length} clusters, final ${processedLocations.length} points`);
 ...
 */
const findLocationClusters = (locations, clusterRadius, minVisitClusterSize, minTravelClusterSize) => {
  console.log(`[LocationEnhancer] Finding clusters: radius=${clusterRadius}m, minVisit=${minVisitClusterSize}, minTravel=${minTravelClusterSize}`);
  ...
  while (i < workingLocations.length) {
    ...
    const visitResult = tryCreateVisitCluster(workingLocations, i, clusterRadius, minVisitClusterSize, minTravelClusterSize);
    
    if (visitResult.success) {
      // Successfully created a visit cluster
      clusters.push(visitResult.cluster);
      console.log(`[LocationEnhancer] Visit cluster: ${visitResult.cluster.pointIndices.length} points at (${visitResult.cluster.centerLat.toFixed(6)}, ${visitResult.cluster.centerLon.toFixed(6)})`);
      i = visitResult.nextIndex;
    } else {
      // Could not create a visit cluster, create a travel cluster instead
      const travelCluster = createTravelCluster(workingLocations, i, minVisitClusterSize);
      clusters.push(travelCluster);
      console.log(`[LocationEnhancer] Travel cluster: ${travelCluster.pointIndices.length} points`);
      i = travelCluster.pointIndices[travelCluster.pointIndices.length - 1] + 1;
    }
  }
  
  console.log(`[LocationEnhancer] Found ${clusters.length} clusters (${clusters.filter(c => c.type === 'visit').length} visits, ${clusters.filter(c => c.type === 'travel').length} travels)`);
  return clusters;
};
 ...
 */
const tryCreateVisitCluster = (workingLocations, startIndex, clusterRadius, minVisitClusterSize, minTravelClusterSize) => {
  const cluster = {
    type: 'visit',
    pointIndices: [startIndex],
    startTime: workingLocations[startIndex].tst || workingLocations[startIndex].timestamp || 0,
    endTime: workingLocations[startIndex].tst || workingLocations[startIndex].timestamp || 0
  };
  ...
  while (j < workingLocations.length) {
    ...
    if (distance <= clusterRadius) {
      ...
      cluster.endTime = workingLocations[j].tst || workingLocations[j].timestamp || 0;
      
      // Recalculate center as average of all points in cluster
      const allLats = cluster.pointIndices.map(idx => workingLocations[idx].lat);
      const allLons = cluster.pointIndices.map(idx => workingLocations[idx].lon);
      ...
      const skipResult = trySkipTransientPoints(workingLocations, j, centerLat, centerLon, clusterRadius, minTravelClusterSize);
 ...
 */
const createTravelCluster = (workingLocations, startIndex, minVisitClusterSize) => {
  // Travel cluster contains minVisitClusterSize-1 points starting from startIndex
  const pointCount = minVisitClusterSize - 1;
  const endIndex = Math.min(startIndex + pointCount - 1, workingLocations.length - 1);
  ...
  const cluster = {
    ...
    startTime: workingLocations[startIndex].tst || workingLocations[startIndex].timestamp || 0,
    endTime: workingLocations[endIndex].tst || workingLocations[endIndex].timestamp || 0
  };
...
const mergeConsecutiveTravelClusters = (clusters) => {
  ...
  while (i < clusters.length) {
    ...
    if (currentCluster.type === 'travel') {
      ...
      if (j > i + 1) {
        console.log(`[LocationEnhancer] Merged ${j - i} consecutive travel clusters into one (${mergedTravelCluster.pointIndices.length} total points)`);
      }
...
const processClusterToLocations = (clusters, originalLocations) => {
  ...
  clusters.forEach((cluster) => {
    if (cluster.type === 'visit') {
      ...
      const clusterPoints = cluster.pointIndices.map(idx => originalLocations[idx]);
      
      // Calculate average location
      const avgLat = clusterPoints.reduce((sum, point) => sum + point.lat, 0) / clusterPoints.length;
      const avgLon = clusterPoints.reduce((sum, point) => sum + point.lon, 0) / clusterPoints.length;
      ...
      console.log(`[LocationEnhancer] Visit ${visitPoint.node_event_id}: ${clusterPoints.length} points merged at (${avgLat.toFixed(6)}, ${avgLon.toFixed(6)})`);
      ...
      console.log(`[LocationEnhancer] Travel ${travelCounter}: ${cluster.pointIndices.length} points retained`);
      travelCounter++;
    }
  });
