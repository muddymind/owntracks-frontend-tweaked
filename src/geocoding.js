  ...
  try {
    ...
    if (data.address) {
      ...
      const facilityFields = [
        addr.building, addr.public_building, addr.amenity, addr.railway,
        addr.aeroway, addr.leisure, addr.tourism, addr.man_made
      ];
      
      for (const field of facilityFields) {
        if (field && field.match(/(station|airport|mall|center|centre|terminal|bahnhof|flygplats|centrum|hauptbahnhof)/i)) {
      ...
      if (!mainFacilityName) {
        if (data.namedetails?.name && data.namedetails.name.match(/(station|airport|mall|center|centre|terminal|bahnhof|flygplats|centrum|hauptbahnhof)/i)) {
          mainFacilityName = data.namedetails.name;
        } else if (data.display_name) {
          const parts = data.display_name.split(',').map(p => p.trim());
          for (const part of parts.slice(0, 3)) {
            if (part.match(/(station|airport|mall|center|centre|terminal|bahnhof|flygplats|centrum|hauptbahnhof)/i)) {
    ...
    if (data.namedetails?.name) {
      ...
      if (firstPart && 
          ...
          !firstPart.match(/^\d+\/\d+$/)) { // Not fractions like "2/3"
    ...
    if (data.address) {
      ...
      const buildingContext = addr.building || addr.public_building || addr.amenity;
      const areaContext = addr.aeroway || addr.railway || addr.leisure || addr.tourism;
      
      // If we have a specific location (like "Track 2") but also building context
      if (locationName && (buildingContext || areaContext || mainFacilityName)) {
        ...
        const isSpecificLocation = specificPatterns.some(pattern => pattern.test(locationName));
        ...
        if (isSpecificLocation) {
          ...
          if (mainFacilityName && 
              ...
              !locationName.toLowerCase().includes(mainFacilityName.toLowerCase().split(' ')[0])) {
            contextName = mainFacilityName;
          } else if (buildingContext && 
                     buildingContext.toLowerCase() !== locationName.toLowerCase() &&
                     !locationName.toLowerCase().includes(buildingContext.toLowerCase().split(' ')[0])) {
            contextName = buildingContext;
          } else if (areaContext && 
                     areaContext.toLowerCase() !== locationName.toLowerCase() &&
                     !locationName.toLowerCase().includes(areaContext.toLowerCase().split(' ')[0])) {
      ...
      if (!locationName) {
        ...
        } else if (addr.building && !addr.building.match(/^(yes|no|true|false|\d+)$/i)) {
          locationName = addr.building;
        } else if (addr.shop && !addr.shop.match(/^(yes|no|true|false|\d+)$/i)) {
          locationName = addr.shop;
        } else if (addr.leisure && !addr.leisure.match(/^(yes|no|true|false|\d+)$/i)) {
          locationName = addr.leisure;
        } else if (addr.tourism && !addr.tourism.match(/^(yes|no|true|false|\d+)$/i)) {
    ...
    if (!locationName && data.address) {
      ...
      if (addr.city && !addressParts.some(part => part.toLowerCase().includes(addr.city.toLowerCase()))) {
        addressParts.push(addr.city);
      } else if (addr.town && !addressParts.some(part => part.toLowerCase().includes(addr.town.toLowerCase()))) {
        addressParts.push(addr.town);
      } else if (addr.village && !addressParts.some(part => part.toLowerCase().includes(addr.village.toLowerCase()))) {
    ...
    console.warn(`[Geocoding] Failed to reverse geocode ${lat}, ${lon}:`, error);
    geocodingCache.set(cacheKey, null);
    return null;
  }
};
...
export const reverseGeocodeWithRateLimit = async (lat, lon) => {
  ...
  return new Promise((resolve, reject) => {
    ...
    const existingRequest = requestQueue.find(req => req.cacheKey === cacheKey);
    if (existingRequest) {
      console.log('[Geocoding] Request already queued for:', cacheKey);
      existingRequest.callbacks.push({ resolve: wrappedResolve, reject: wrappedReject });
      return;
    }
    
    // Check queue size limit
    if (requestQueue.length >= MAX_QUEUE_SIZE) {
      console.warn('[Geocoding] Queue size limit reached, rejecting request for:', cacheKey);
    ...
    console.log('[Geocoding] Added to queue:', cacheKey, 'Queue length:', requestQueue.length);
...
async function processRequestQueue() {
  ...
  console.log('[Geocoding] Starting queue processing, queue length:', requestQueue.length);
  ...
  while (requestQueue.length > 0 && processedCount < maxProcessingAttempts) {
    ...
    try {
      ...
      if (consecutiveErrors >= MAX_CONSECUTIVE_ERRORS) {
        console.error('[Geocoding] Too many consecutive errors, opening circuit breaker');
  ...
  if (processedCount >= maxProcessingAttempts) {
    console.warn('[Geocoding] Maximum processing attempts reached, clearing remaining queue');
    while (requestQueue.length > 0) {
      const req = requestQueue.shift();
      req.callbacks.forEach(cb => {
        try {
  ...
  console.log('[Geocoding] Queue processing complete, processed:', processedCount);
}
